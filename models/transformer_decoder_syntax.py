...
class DynamicSpeaker(nn.Module):
    def __init__(self, cfg):
        ...
        if self.share_wd_cls_weight:
            ...
            self.logit_dep = nn.Linear(self.hidden_size, 49)
    ...
    def _forward(self,
                 encoder_output, seq, dep, mask, labels_with_ignore=None, deps_with_ignore=None):
        ...
        prediction_dep_scores = self.logit_dep(decoder_outputs)
        caption_loss = 0.
        dep_loss = 0.
        ...
        if deps_with_ignore is not None:
            dep_loss = self.loss_func(prediction_dep_scores.view(-1, 49),
                                          deps_with_ignore.view(-1))
        return caption_loss, prediction_scores, attention_weight, dep_loss
