...
while t < cfg.train.max_iter:
    ...
    for i, batch in enumerate(train_loader):
        ...
        d_img_paths, nsc_img_paths, sc_img_paths, deps, deps_with_ignore, \
        neg_deps, neg_deps_with_ignore = batch
        ...
        deps = deps.squeeze(1)
        deps_with_ignore = deps_with_ignore.squeeze(1)
        neg_deps = neg_deps.squeeze(1)
        neg_deps_with_ignore = neg_deps_with_ignore.squeeze(1)
        ...
        deps, deps_with_ignore, neg_deps, neg_deps_with_ignore = deps.to(device), \
        deps_with_ignore.to(device), neg_deps.to(device), neg_deps_with_ignore.to(device)
        optimizer.zero_grad()
        ...
        loss_pos, _, attention_weight_pos, dep_loss_pos = speaker._forward(encoder_output_pos,
                                              labels, deps, masks, labels_with_ignore=labels_with_ignore, deps_with_ignore=deps_with_ignore)

        loss_neg, _, attention_weight_neg, dep_loss_neg = speaker._forward(encoder_output_neg,
                                              no_chg_labels, neg_deps, no_chg_masks, labels_with_ignore=no_chg_labels_with_ignore, deps_with_ignore=neg_deps_with_ignore)
        ...
        dep_loss = 0.5 * dep_loss_pos + 0.5 * dep_loss_neg
        dep_loss_val = dep_loss.item()

        total_loss = speaker_loss + 0.01 * dep_loss + 2.5e-03 * att_sum

        total_loss_val = total_loss.item()

        speaker_loss_avg.update(speaker_loss_val, 2 * batch_size)
        dep_loss_avg.update(dep_loss_val, 2 * batch_size)
        ...
        stats['dep_loss'] = dep_loss_val
        stats['avg_dep_loss'] = dep_loss_avg.avg
        ...
        if t % cfg.train.snapshot_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_i, val_batch in enumerate(val_loader):
                    d_feats, nsc_feats, sc_feats, \
                    labels, labels_with_ignore, no_chg_labels, no_chg_labels_with_ignore, masks, no_chg_masks, aux_labels_pos, aux_labels_neg, \
                    d_img_paths, nsc_img_paths, sc_img_paths, deps, deps_with_ignore, neg_deps, neg_deps_with_ignore = val_batch
